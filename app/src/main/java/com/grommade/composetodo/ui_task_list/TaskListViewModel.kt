...
class TaskListViewModel @Inject constructor(
    ...
    private val shownTasks = repoSingleTask.allTasks
        .map { getTasksToShow(if (mode == ModeTaskList.SELECT_CATALOG) getOpenGroups(it) else it) }

    private var selectedTasks = MutableStateFlow<List<Task>>(emptyList())

    private val actionMode = MutableStateFlow(false)
    ...
    private val defaultTitle: Int = when (taskType) {
...
//    val shownTasks: StateFlow<List<TaskItem>> = tasks.combine(selectedTasks) { tasks, _ -> tasks }
    ...
    init {
        viewModelScope.launch {
            pendingActions.collect { action ->
                when (action) {
                    TaskListActions.CloseActionMode -> closeActionMode()
...
//
    ...
    fun openCloseGroup(task: Task) {
        if (task.group) {
            task.copy(groupOpen = !task.groupOpen).save()
        }
    }

    private fun getOpenGroups(tasks: List<Task>): List<Task> =
        tasks.filter { it.group }.map { it.copy(groupOpen = true) }

//    private fun List<Task>.toTaskItem(): List<TaskItem> = map { task ->
//        TaskItem(
//            task = task,
//            level = task.getLevel(tasks.value),
...
//    private fun getTask(id: Long): Task? = shownTasks.value.find { it.id == id }
